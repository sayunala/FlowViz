...
class igwPresetDialogTableModel : public QAbstractTableModel
{
  typedef QAbstractTableModel Superclass;
  igwPresetToPixmap PixmapRenderer;
...
public:
  vtkSmartPointer<igwTransferFunctionPresets> Presets;
  igwPresetGroupsManager* GroupManager;

  igwPresetDialogTableModel(QObject* parentObject)
    : Superclass(parentObject)
  {
    this->Presets = igwTransferFunctionPresets::GetInstance();
    this->Pixmaps.reserve(this->Presets->GetNumberOfPresets());
    auto number = this->Presets->GetNumberOfPresets();
//    this->GroupManager = new igwPresetGroupsManager(this);
//    QFile file(":/Color/src/ColorMap/ColorManager/pqPresetGroups.json");
//    file.open(QIODevice::ReadOnly | QIODevice::Text);
//    QJsonParseError error;
    //    QJsonDocument document = QJsonDocument::fromJson(file.readAll(), &error);
    this->GroupManager = igwColorMapCore::GetInstance()->GetGroupManager();
//    this->GroupManager->loadGroups(file.readAll());
//    file.close();
...
//    });
  ...
  ~igwPresetDialogTableModel() override {
      delete GroupManager;
  }
  ...
  {
    this->beginResetModel();
    std::vector<igwTransferFunctionPresets::ImportedPreset> importedPresets;
...
private:
  Q_DISABLE_COPY(igwPresetDialogTableModel)
};

class igwPresetDialogProxyModel : public QSortFilterProxyModel
{
  typedef QSortFilterProxyModel Superclass;
  igwPresetDialog::Modes Mode;
  int CurrentGroupColumn;

public:
  igwPresetDialogProxyModel(igwPresetDialog::Modes m, QObject* parentObject = nullptr)
    : Superclass(parentObject)
    , Mode(m)
    , CurrentGroupColumn(0)
  {
  }
  ~igwPresetDialogProxyModel() override = default;

  igwPresetDialog::Modes mode() { return this->Mode; }
  void setMode(igwPresetDialog::Modes m)
  {
    this->Mode = m;
...
protected:
  bool filterAcceptsRow(int sourceRow, const QModelIndex& sourceParent) const override
  {
    ...
    switch (this->Mode)
    {
      case igwPresetDialog::SHOW_ALL:
        return true;

      case igwPresetDialog::SHOW_INDEXED_COLORS_ONLY:
        return source->data(idx, Qt::UserRole).toBool();

      case igwPresetDialog::SHOW_NON_INDEXED_COLORS_ONLY:
        return !source->data(idx, Qt::UserRole).toBool();
    }
    return false;
  }
...
private:
  Q_DISABLE_COPY(igwPresetDialogProxyModel)
};

class igwPresetDialogReflowModel : public QAbstractProxyModel
{
  typedef QAbstractProxyModel Superclass;
  int NumColumns;

public:
  igwPresetDialogReflowModel(int cols, QObject* parentObject = nullptr)
    : Superclass(parentObject)
    , NumColumns(cols)
  {
  }
  ~igwPresetDialogReflowModel() override = default;
...
private:
  Q_DISABLE_COPY(igwPresetDialogReflowModel);
};

class igwPresetDialog::pqInternals
{
public:
  Ui::igwPresetDialog Ui;
  QPointer<igwPresetDialogTableModel> Model;
  QPointer<igwPresetDialogProxyModel> ProxyModel;
  QPointer<igwPresetDialogReflowModel> ReflowModel;
  int CurrentGroupColumn;

  pqInternals(igwPresetDialog::Modes mode, igwPresetDialog* self)
    : Model(new igwPresetDialogTableModel(self))
    , ProxyModel(new igwPresetDialogProxyModel(mode, self))
    , ReflowModel(new igwPresetDialogReflowModel(2, self))
  {
...
//-----------------------------------------------------------------------------
igwPresetDialog::igwPresetDialog(QWidget* parentObject, igwPresetDialog::Modes mode)
  : Superclass(parentObject)
  , Internals(new igwPresetDialog::pqInternals(mode, this))
{
  const Ui::igwPresetDialog& ui = this->Internals->Ui;
...
//    ui.searchBox, &pqSearchBox::textChanged, this, &igwPresetDialog::updateFiltering);
...
//  this->connect(
//    groupMgr, &pqPresetGroupsManager::groupsUpdated, this, &igwPresetDialog::updateGroups);
  this->connect(this, SIGNAL(rejected()), SLOT(close()));
//  this->updateGroups();
  auto defaultColumn = groupMgr->groupNames().indexOf("Default") + 1;
  this->Internals->ProxyModel->setCurrentGroupColumn(defaultColumn);
}

//-----------------------------------------------------------------------------
igwPresetDialog::~igwPresetDialog() = default;

//-----------------------------------------------------------------------------
//void igwPresetDialog::updateGroups()
//{
...
//-----------------------------------------------------------------------------
//void igwPresetDialog::updateFiltering()
//{
...
//-----------------------------------------------------------------------------
void igwPresetDialog::setMode(Modes mode)
{
  this->Internals->setMode(mode);
}
//-----------------------------------------------------------------------------
//void igwPresetDialog::setCustomizableLoadColors(bool state, bool defaultValue)
//{
//  const Ui::igwPresetDialog& ui = this->Internals->Ui;
...
//-----------------------------------------------------------------------------
//void igwPresetDialog::setCustomizableLoadAnnotations(bool state, bool defaultValue)
//{
//  const Ui::igwPresetDialog& ui = this->Internals->Ui;
...
//-----------------------------------------------------------------------------
//void igwPresetDialog::setCustomizableAnnotationsRegexp(bool state, bool defaultValue)
//{
//  const Ui::igwPresetDialog& ui = this->Internals->Ui;
...
//-----------------------------------------------------------------------------
//void igwPresetDialog::setCustomizableLoadOpacities(bool state, bool defaultValue)
//{
//  const Ui::igwPresetDialog& ui = this->Internals->Ui;
...
//-----------------------------------------------------------------------------
//void igwPresetDialog::setCustomizableUsePresetRange(bool state, bool defaultValue)
//{
//  const Ui::igwPresetDialog& ui = this->Internals->Ui;
...
//-----------------------------------------------------------------------------
void igwPresetDialog::setCurrentPreset(const char* presetName)
{
...
//-----------------------------------------------------------------------------
void igwPresetDialog::updateEnabledStateForSelection()
{
  const Ui::igwPresetDialog& ui = this->Internals->Ui;
...
//-----------------------------------------------------------------------------
void igwPresetDialog::updateForSelectedIndex(const QModelIndex& proxyIndex)
{
  ...
  const Ui::igwPresetDialog& ui = internals.Ui;
...
//-----------------------------------------------------------------------------
void igwPresetDialog::triggerApply(const QModelIndex& _proxyIndex)
{
  const pqInternals& internals = *this->Internals;
  const Ui::igwPresetDialog& ui = this->Internals->Ui;
...
//-----------------------------------------------------------------------------
void igwPresetDialog::removePreset(const QModelIndex& _proxyIndex)
{
  const pqInternals& internals = *this->Internals;
  const Ui::igwPresetDialog& ui = this->Internals->Ui;
...
//-----------------------------------------------------------------------------
const Json::Value& igwPresetDialog::currentPreset()
{
  const pqInternals& internals = *this->Internals;
  const Ui::igwPresetDialog& ui = this->Internals->Ui;
...
//-----------------------------------------------------------------------------
//bool igwPresetDialog::loadColors() const
//{
//  const Ui::igwPresetDialog& ui = this->Internals->Ui;
...
////-----------------------------------------------------------------------------
//bool igwPresetDialog::loadOpacities() const
//{
//  const Ui::igwPresetDialog& ui = this->Internals->Ui;
...
////-----------------------------------------------------------------------------
//bool igwPresetDialog::loadAnnotations() const
//{
//  const Ui::igwPresetDialog& ui = this->Internals->Ui;
...
////-----------------------------------------------------------------------------
//bool igwPresetDialog::usePresetRange() const
//{
//  const Ui::igwPresetDialog& ui = this->Internals->Ui;
//  return ui.usePresetRange->isEnabled() ? ui.usePresetRange->isChecked() : false;
//}
//-----------------------------------------------------------------------------
void igwPresetDialog::importPresets()
{
  QFileDialog dialog;
  dialog.setObjectName("ImportPresets");
  dialog.setFileMode(QFileDialog::ExistingFile);
  dialog.setNameFilter(tr("Color Map(*.json)"));
//  if (dialog.exec() == QDialog::Accepted /*&& !dialog.getSelectedFiles().empty()*/)
//  {
    QString filename = dialog.getOpenFileName(nullptr, "Import Preset", "", "Color Map (*.json)");
    const pqInternals& internals = *this->Internals;
...
void igwPresetDialog::exportPresets()
{


//  QFileDialog dialog/*(nullptr, this, tr("Export Preset(s)"), QString(),
//    tr("ParaView Color/Opacity Presets") + QString(" (*.json);;") + tr("All Files") +
//      QString(" (*)"),
//    false)*/;
//  dialog.setNameFilter(tr("iGreatWorks Color/Presets") + QString(" (*.json);;"));
...
//  }
    ...
    QString defaultFileName = "ColorMap.json";
    QString filename = QFileDialog::getSaveFileName(nullptr, "Export Preset", defaultFileName, "Color Map (*.json)");
...
//-----------------------------------------------------------------------------
void igwPresetDialog::setPresetIsAdvanced(int newState)
{
...
void igwPresetDialog::ResetTable()
{
    const pqInternals& internals = *this->Internals;
    //        int oldCount = internals.Model->rowCount(QModelIndex());
    internals.Model->reset();
}

//-----------------------------------------------------------------------------
//QRegularExpression igwPresetDialog::regularExpression()
//{
