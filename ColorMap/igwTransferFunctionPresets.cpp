/*=========================================================================
  ...
  Program:   ParaView
  Module:    igwTransferFunctionPresets.cxx
...
#include "igwTransferFunctionPresetsBuiltin.h"
...
vtkSmartPointer<igwTransferFunctionPresets> igwTransferFunctionPresets::Instance;

class igwTransferFunctionPresets::vtkInternals
{
public:
  vtkInternals()
    ...
    if (importedPresets != nullptr)
    {
      ...
      std::transform(root.begin(), root.end(), std::back_inserter(*importedPresets),
        [&](Json::Value const& preset) {
          auto result = igwTransferFunctionPresets::ImportedPreset{};
...
private:
  ...
  void LoadBuiltinPresets()
  {
    ...
    char* rawJSON = igwTransferFunctionPresetsColorMapsJSON();
    Json::CharReaderBuilder builder;
    builder["collectComments"] = false;
...
igwTransferFunctionPresets* igwTransferFunctionPresets::New()
{
  auto instance = igwTransferFunctionPresets::GetInstance();
  instance->Register(nullptr);
  return instance;
}

//----------------------------------------------------------------------------
igwTransferFunctionPresets::igwTransferFunctionPresets()
  : Internals(new igwTransferFunctionPresets::vtkInternals())
{
}
...
//----------------------------------------------------------------------------
igwTransferFunctionPresets* igwTransferFunctionPresets::GetInstance()
{
  if (igwTransferFunctionPresets::Instance.GetPointer() == nullptr)
  {
    auto presets = new igwTransferFunctionPresets();
    presets->InitializeObjectBase();
    igwTransferFunctionPresets::Instance.TakeReference(presets);
  }
...
//----------------------------------------------------------------------------
std::string igwTransferFunctionPresets::GetPresetAsString(unsigned int index)
{
...
//----------------------------------------------------------------------------
const Json::Value& igwTransferFunctionPresets::GetPreset(unsigned int index)
{
...
//----------------------------------------------------------------------------
const Json::Value& igwTransferFunctionPresets::GetFirstPresetWithName(const char* name, int& idx)
{
  static Json::Value nullValue;
  if (name == nullptr)
  {
...
//----------------------------------------------------------------------------
const Json::Value& igwTransferFunctionPresets::GetFirstPresetWithName(const char* name)
{
  int idx;
...
//----------------------------------------------------------------------------
bool igwTransferFunctionPresets::HasPreset(const char* name)
{
  return !this->GetFirstPresetWithName(name).isNull();
}

//----------------------------------------------------------------------------
std::string igwTransferFunctionPresets::GetPresetName(unsigned int index)
{
  const std::vector<Json::Value>& presets = this->Internals->GetPresets();
...
//----------------------------------------------------------------------------
bool igwTransferFunctionPresets::RenamePreset(unsigned int index, const char* newname)
{
  return this->Internals->RenamePreset(index, newname);
}

//----------------------------------------------------------------------------
bool igwTransferFunctionPresets::AddPreset(const char* name, const std::string& preset)
{
  if (!name)
  {
...
//----------------------------------------------------------------------------
bool igwTransferFunctionPresets::SetPreset(const char* name, const Json::Value& preset)
{
  int idx;
...
//----------------------------------------------------------------------------
bool igwTransferFunctionPresets::AddPreset(const char* name, const Json::Value& preset)
{
...
//----------------------------------------------------------------------------
std::string igwTransferFunctionPresets::AddUniquePreset(
...
//----------------------------------------------------------------------------
bool igwTransferFunctionPresets::RemovePreset(unsigned int index)
{
  return this->Internals->RemovePreset(index);
}

//----------------------------------------------------------------------------
unsigned int igwTransferFunctionPresets::GetNumberOfPresets()
{
  return static_cast<unsigned int>(this->Internals->GetPresets().size());
}

//----------------------------------------------------------------------------
bool igwTransferFunctionPresets::GetPresetHasOpacities(const Json::Value& preset)
{
  return (!preset.empty() && preset.isMember("Points"));
}

//----------------------------------------------------------------------------
bool igwTransferFunctionPresets::GetPresetHasIndexedColors(const Json::Value& preset)
{
  return (!preset.empty() && preset.isMember("IndexedColors"));
}

//----------------------------------------------------------------------------
bool igwTransferFunctionPresets::GetPresetHasAnnotations(const Json::Value& preset)
{
  return (!preset.empty() && preset.isMember("Annotations"));
}

//----------------------------------------------------------------------------
bool igwTransferFunctionPresets::IsPresetBuiltin(unsigned int index)
{
...
//----------------------------------------------------------------------------
bool igwTransferFunctionPresets::ImportPresets(const char* filename)
{
  return this->ImportPresets(filename, nullptr);
}

//----------------------------------------------------------------------------
bool igwTransferFunctionPresets::ImportPresets(
...
//----------------------------------------------------------------------------
bool igwTransferFunctionPresets::ImportPresets(const Json::Value& presets)
{
...
//----------------------------------------------------------------------------
void igwTransferFunctionPresets::PrintSelf(ostream& os, vtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//----------------------------------------------------------------------------
void igwTransferFunctionPresets::ReloadPresets()
{
