...
//-----------------------------------------------------------------------------
vtkStandardNewMacro(igwCSVWriter);

//-----------------------------------------------------------------------------
vtkCxxSetObjectMacro(igwCSVWriter, Controller, vtkMultiProcessController);

//-----------------------------------------------------------------------------
igwCSVWriter::igwCSVWriter()
{
...
//-----------------------------------------------------------------------------
int igwCSVWriter::FillInputPortInformation(int vtkNotUsed(port), vtkInformation* info)
{
...
//------------------------------------------------------------------------------
int igwCSVWriter::RequestInformation(vtkInformation* vtkNotUsed(request),
...
//----------------------------------------------------------------------------
int igwCSVWriter::RequestUpdateExtent(vtkInformation* vtkNotUsed(request),
  vtkInformationVector** inputVector, vtkInformationVector* vtkNotUsed(outputVector))
{
  if (!this->TimeValues)
  {
...
//-----------------------------------------------------------------------------
int igwCSVWriter::RequestData(vtkInformation* request,
...
//----------------------------------------------------------------------------
int igwCSVWriter::ProcessRequest(
...
struct AbstractStreamWorker
{
  ...
  virtual void operator()(ostream& stream, igwCSVWriter* writer, vtkIdType index) = 0;
...
struct DataToStreamWorker : public AbstractStreamWorker
{
  ...
  void operator()(ostream& stream, igwCSVWriter* vtkNotUsed(writer), vtkIdType index) override
  {
...
struct DataToStreamWorker<vtkStringArray> : public AbstractStreamWorker
{
  ...
  void operator()(ostream& stream, igwCSVWriter* writer, vtkIdType index) override
  {
...
public:
  ...
  void WriteHeader(vtkTable* table, igwCSVWriter* self, OpenMode mode)
  {
    this->WriteHeader(table->GetRowData(), self, mode);
  }

  void WriteHeader(vtkDataSetAttributes* dsa, igwCSVWriter* self, OpenMode mode)
  {
    if (OpenMode::Write == mode)
    {
  ...
  void InitializeStreamWorkers(vtkDataSetAttributes* dsa, igwCSVWriter* self)
  {
  ...
  void WriteData(vtkTable* table, igwCSVWriter* self)
  {
    this->InitializeStreamWorkers(table->GetRowData(), self);
    this->WriteData(table->GetRowData(), self);
  }

  void WriteData(vtkDataSetAttributes* dsa, igwCSVWriter* self)
  {
...
//-----------------------------------------------------------------------------
std::string igwCSVWriter::GetString(std::string string)
{
...
void igwCSVWriter::WriteData()
{
  ...
  if (table == nullptr)
  {
    ...
    if (table == nullptr)
    {
      vtkNew<vtkMergeTables> mergeTables;
  ...
  {
    igwCSVWriter::CSVFile file(timeStep, time);
...
//-----------------------------------------------------------------------------
void igwCSVWriter::PrintSelf(ostream& os, vtkIndent indent)
{
